...
public class UltimateTicTacToeBoard
{
    ...
    public IEnumerator ActOnEachBoardCellWithDelay(Action<TicTacToeBoardCell> func, float delay, bool reverseOrder)
    {
        if (reverseOrder)
        {
            for (int i = boards.GetLength(0) - 1; i >= 0; i--)
            {
                ...
                {
                    boards[i, p].ActOnEachBoardCellWithDelay(func, delay, reverseOrder);
    ...
    public IEnumerator Generate()
    {
        ...
        glGroup.constraintCount = 3;
        glGroup.GetComponent<RectTransform>().sizeDelta = Vector2.one * 3 * cellSize;

        boards = new TicTacToeBoard[3, 3];
        for (int i = 0; i < boards.GetLength(0); i++)
        {
            for (int p = 0; p < boards.GetLongLength(0); p++)
            {
                TicTacToeBoard board = GameObject.Instantiate(boardPrefab, boardParentTransform);
                boards[i, p] = board;
                board.name += "<" + i + ", " + p + ">";
    ...
    private bool CheckForHorizontalWin(TicTacToeBoardCell cellToCheck)
    {
        ...
        for (int i = 0; i < boards.GetLength(0); i++)
        {
            TicTacToeBoardCell currentlyChecking = boards[row, i];
